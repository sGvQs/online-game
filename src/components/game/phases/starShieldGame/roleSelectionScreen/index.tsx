...
interface RoleSelectionScreenProps {
	...
	onShooterLoadoutUpdate?: (updates: {
...
export function RoleSelectionScreen({
	...
	const displayShooterProgress =
		shooterProgress ?? (myRole === "SHOOTER" ? currentUserProgress : null);
	const displayTypistProgress =
		typistProgress ?? (myRole === "TYPIST" ? currentUserProgress : null);
			...
			<div className="relative z-10 w-full max-w-2xl mx-auto px-6 py-10 flex flex-col gap-7">
				...
				<div className="grid grid-cols-[1fr_1fr] gap-5 items-stretch">
					...
					>
						<Typography
							...
							className={styles.difficultyCardTitle()}
						>
							むずかしさ
						</Typography>
								...
								return (
									<Button
										...
										style={{
											...
											["--diff-color" as string]: isActive
												? meta.text
												: isLocked
													? COLORS.WHITE_15
													: COLORS.WHITE_2,
											["--diff-rate-color" as string]: isActive
												? meta.text
												: COLORS.WHITE_15,
										}}
									>
										...
										<Typography
											...
											as="span"
											font="cherry-bomb-one"
											className="font-bold flex-1 text-left"
										>
											{meta.label}
										</Typography>
										{isLocked && (
											<Typography
												variant="caption"
												as="span"
												className={styles.hellLockLabel()}
											>
												解放条件
											</Typography>
										)}
						...
						{(["SHOOTER", "TYPIST"] as const).map((r) => {
							...
							return (
								<div
									...
									onKeyDown={(e) =>
										(e.key === "Enter" || e.key === " ") &&
										(e.preventDefault(), onRoleChange(r))
									}
									className={styles.roleButton()}
									style={{
										["--role-bg" as string]: isSelected
											? meta.bg
											: "rgba(255,255,255,0.02)",
										["--role-border" as string]: isSelected
											? `1.5px solid ${meta.border}`
											: "1.5px solid rgba(255,255,255,0.07)",
										["--role-glow" as string]: isSelected ? meta.glow : "none",
										["--role-icon-filter" as string]: isSelected
											? "none"
											: "opacity(0.35)",
										["--role-title-color" as string]: isSelected
											? meta.text
											: COLORS.WHITE_35,
										["--role-desc-color" as string]: isSelected
											? meta.text
											: COLORS.WHITE_25,
										["--role-detail-color" as string]: isSelected
											? `${meta.text}99`
											: COLORS.WHITE_15,
									}}
								>
									...
									{isSelected && r === "SHOOTER" && displayShooterProgress && (
										<div
											className="mt-3 pt-3 border-t border-white/6 min-h-22"
											onClick={(e) => e.stopPropagation()}
										>
											<Typography
												variant="caption"
												as="p"
												className="text-[10px] mb-1.5 opacity-70"
											>
												選択している色
											</Typography>
												...
												<div className="flex items-center gap-2">
													...
													<div className="flex-1 min-w-0 h-1.5 rounded-full bg-white/5 overflow-hidden">
														<div
															...
															style={{
																width: `${((displayTypistProgress.starHpLevel ?? 1) / 5) * 100}%`,
															}}
														/>
													</div>
													<Typography
												...
												<div className="flex flex-col gap-1">
													<div className="flex items-center gap-2">
														<Typography
															variant="caption"
															as="span"
															className="text-[10px] opacity-80 shrink-0"
														>
															...
															<>
																<div className="flex-1 min-w-0 h-1.5 rounded-full bg-white/5 overflow-hidden">
																	<div
																		...
																		style={{
																			width: `${(displayTypistProgress.healLevel / 6) * 100}%`,
																		}}
																	/>
																</div>
																<Typography
																...
																>
																	...
																	{displayTypistProgress.healLevel === 6
																		? "max"
																		: displayTypistProgress.healLevel}
																	/6
																</Typography>
															</>
														)}
				...
				<motion.div
					...
					style={{
						...
						["--status-panel-border" as string]: canProceed
							? "1px solid rgba(129,140,248,0.15)"
							: "1px solid rgba(248,113,113,0.4)",
						["--status-message-color" as string]: canProceed
						...
						{room.users.map((u: RoomUserWithReadyStatus) => {
							...
							return (
								<div
									...
									style={{
										["--status-dot-color" as string]: roleMeta
											? roleMeta.text
											: COLORS.WHITE_15,
