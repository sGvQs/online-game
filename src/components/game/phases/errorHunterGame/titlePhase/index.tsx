				...
				<div className={styles.modalInner()}>
					<div className={styles.titlebar()}>
						<Typography
							variant="label"
							as="span"
							className={styles.titlebarText()}
						>
							ERROR HUNTER
						</Typography>
						{isHost && (
							<div className={styles.titlebarButtons()}>
								<Win95TitleBarButton onClick={onClose}>×</Win95TitleBarButton>
							</div>
						)}
					</div>
					...
					<div className={styles.twoColumn()}>
								...
								{!showDescription ? (
										...
										<div className={styles.playerStatusSection()}>
											...
											<div className={styles.playerListbox()}>
												{[...room.users]
													.sort((a, b) => {
														const rankA =
															rankings.find((r) => r.userId === a.userId)
																?.rank ?? Infinity;
														const rankB =
															rankings.find((r) => r.userId === b.userId)
																?.rank ?? Infinity;
														return rankA - rankB;
													})
													.map((roomUser: RoomUserWithReadyStatus) => {
														...
														const faceIcon =
															roomUser.user?.faceIcon ?? DEFAULT_FACE_ICON;
									...
									<div className={styles.descriptionContent()}>
										...
										<div className={styles.descriptionText()}>
											ERROR
											HUNTERは、画面に出現する47個のエラーモーダルを素早く閉じる反射神経ゲームです。
											<br />
											<br />
											<strong>ルール:</strong>
											<br />
											・全47個のエラーが一斉に画面上に出現します
											<br />
											・各プレイヤーは素早くエラーの×ボタンをクリック
											<br />
											・最も多くのエラーを閉じたプレイヤーが勝利
											<br />
											・全員で協力して全てのエラーを閉じましょう！
											<br />
											・何回か押さないと閉じないものもあるよ
											<br />
											・↑はprismaの限界だから許してね
						...
						<div className={styles.rightPanel()}>
							<Win95Button
								className={cn(
									styles.panelButton(),
									showDescription && styles.buttonPressed(),
								)}
							...
							{isHost && (
								<Win95Button className={styles.panelButton()} onClick={onClose}>
									Close
								</Win95Button>
							)}

							{!isHost && (
								<Win95Button className={styles.panelButton()} onClick={onLeave}>
									LEAVE
								</Win95Button>
							)}
						</div>
