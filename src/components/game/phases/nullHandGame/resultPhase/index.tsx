...
export function ResultPhase({
	...
	const myHandData = jankenEvent.guestHands.find(
		(gh: (typeof jankenEvent.guestHands)[number]) =>
			gh.userId === currentUserId,
	);
	const myHand = myHandData?.hand as HandType | undefined;

	const hostUser = roomUsers.find(
		(u) => u.userId === jankenEvent.currentHostId,
	);
	...
	const isNullHand =
		...
		guestHands.every(
			(gh: GuestHandItem) =>
				judgeHand(hostHand, gh.hand as HandType) === "DRAW",
		);
	const isHostPerfectWin =
		guestCount > 1 && !isNullHand && !hasGuestWin && !hasDraw;
				...
				<div className="flex justify-center items-center gap-2 text-gray-400 text-xs font-sans tracking-widest">
					<div
						className={cn(
							...
							earnedPoints > 0 ? "bg-[#44FFFF]" : "bg-gray-600",
						)}
					/>
								...
								<div className="flex items-center justify-center gap-12 mb-8">
									<div className="flex flex-col items-center">
										<div className="flex flex-col items-center gap-2">
											<PlayerFaceIcon
												faceIcon={hostUser?.user?.faceIcon}
												size="lg"
											/>
											<div
												className={cn(styles.playerName(), "text-[#FF4444]")}
											>
												{hostName}
											</div>
										</div>
									...
									<div className="text-gray-800 font-black text-4xl italic px-4 translate-y-4">
										VS
									</div>
									...
									<div className="flex flex-col items-center">
										<div className="flex flex-col items-center gap-2">
											<PlayerFaceIcon
												faceIcon={currentUserData?.user?.faceIcon}
												size="lg"
											/>
											<div
												className={cn(styles.playerName(), styles.myselfName())}
												style={{ ["--result-user-color" as string]: userColor }}
											>
												YOU
											</div>
										</div>
						...
						<div className="flex-1 flex flex-col items-center justify-center">
							...
							<div className="relative w-120 h-48 flex items-center justify-center">
									...
									<div className="flex flex-col items-center gap-1 translate-y-2">
										...
										<div className="h-6">
											<Typography
												...
												as="p"
												font="sans"
												className="text-[#44FFFF] font-black tracking-widest text-center translate-y-2 px-3 py-2"
											>
												SYSTEM SELECTION
											</Typography>
										</div>
								...
								<div className="absolute inset-0 flex items-center justify-between px-4">
									...
									>
										<div className="flex items-end gap-2">
											...
											<div
												...
												style={{
													["--result-choice-color" as string]: isCurrentHost
														? userColor || "#FF4444"
														: "#FF4444",
												}}
											>
									...
									<div className="order-2 text-gray-500 font-black text-xl translate-y-4">
										OR
									</div>
		...
		>
			<div className="flex flex-col gap-4 h-full">
				...
				<CurrentScores
					...
					userColor={userColor}
				/>
				<RewardSystem isHost={isCurrentHost} userColor={userColor} size="md" />
			</div>
		</motion.div>
	);
