...
export function DealPhase({
	...
	const HandDisplay = ({
		...
		<div
			className={`flex flex-col items-center gap-3 p-6 border ${color === "cyan" ? "border-[#44FFFF]/30" : "border-[#FF4444]/30"} bg-black/40`}
		>
			<div
				className={`text-xs font-black tracking-[0.3em] uppercase ${color === "cyan" ? "text-[#44FFFF]" : "text-[#FF4444]"}`}
			>
				{label}
			</div>
		...
		>
			...
			<div className="flex-1 flex flex-col items-center justify-center gap-8">
				{!systemHandsRevealed ? (
					...
					>
						<HandDisplay
							hand={jankenEvent.systemRealHand as HandType}
							label="REAL"
							color="cyan"
						/>
						<HandDisplay
							hand={jankenEvent.systemBluffHand as HandType}
							label="BLUFF"
							color="red"
						/>
					</motion.div>
				...
				<div className="max-w-sm text-center space-y-2 text-xs text-gray-500 leading-relaxed border border-gray-800 p-4 bg-black/20">
					...
					<Typography variant="body" font="sans">
						<span className="text-[#44FFFF]">REAL</span> は{" "}
						<span className="text-[#FF4444]">BLUFF</span> に必ず勝ちます。
					</Typography>
					<Typography variant="body" font="sans">
						ホストは <span className="text-white font-bold">STAY</span>
						（REALを出す）か
						<span className="text-white font-bold"> REVERSE</span>
						（BLUFFを出す）かを選択します。
					</Typography>
					<Typography variant="body" font="sans">
						過去のホストの{" "}
						<span className="text-white font-bold">REVERSE RATE</span>{" "}
						を参考に手を選んでください。
					</Typography>
				</div>
			</div>
				...
				{isCurrentHost ? (
					<div className="bg-[#051a1a] border border-[#44FFFF]/30 rounded-xl p-4">
						<div className="flex items-center gap-2 border-b border-[#44FFFF]/30 pb-2 mb-2">
							...
							<span className="text-[10px] font-black tracking-widest text-[#44FFFF] uppercase">
								YOUR TURN
							</span>
							<span className="text-[8px] bg-[#44FFFF] text-black px-1 font-black rounded-[2px] ml-auto">
								HOST
							</span>
						</div>
						<div className="space-y-2">
							<div className="text-[10px] text-gray-400 leading-relaxed">
								<span className="text-[#44FFFF] font-bold">STEP 1:</span>{" "}
								手を確認し「NEXT」で進む
							</div>
						</div>
					</div>
