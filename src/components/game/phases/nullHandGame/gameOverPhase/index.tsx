	...
	const MainArea = () => (
		<div className={styles.mainArea()}>
			...
			<div className="text-center mt-12">
				{(() => {
					const currentUserScore = currentScores.find(
						(s) => s.userId === currentUserId,
					);
					if (!currentUserScore) return null;

					const oldRanking = initialRankings.find(
						(r) => r.userId === currentUserId,
					);
					const oldRank = oldRanking?.rank;
					const oldPoints = oldRanking?.points;

					const newRanking = newRankings.find(
						(r) => r.userId === currentUserId,
					);
					...
					const isSingleWinner =
						currentScores.filter(
							(s) =>
								s.points === Math.max(...currentScores.map((sc) => sc.points)),
						).length === 1;
					...
					return (
						<div className="mb-12">
							<div className="w-48 h-48 mx-auto mb-4">
								<Hand3D
									handType={
										currentScores.find((s) => s.userId === currentUserId)
											?.userId
											? HandType.ROCK
											: null
									} // とりあえずROCKかタイトル手を表示
							...
							<div className="flex justify-center items-end mb-2 gap-1">
								<PlayerFaceIcon
									...
									size="lg"
								/>
								<div className="text-[#44FFFF] text-xl font-bold tracking-widest">
									のランキング
								</div>
							</div>
								...
								<div className="mt-4 flex items-center justify-center gap-4 text-xl font-sans pb-2">
									...
									<div className="flex items-center gap-4">
										...
										<Typography
											...
											as="span"
											font="sans"
											className="text-[#44FFFF]"
										>
											{Math.floor(newPoints)}pt
										</Typography>
									</div>
								</div>
							)}
				...
				<div className="flex w-full items-center justify-center">
					<GameButton onClick={onFinish}>BACK TO TITLE</GameButton>
				</div>
			</div>
		</div>
	);
