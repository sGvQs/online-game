...
export function Win95Dialog({
	...
	const styles = win95Dialog();
	const innerClass =
		innerClassName === "error" ? styles.innerError() : styles.inner();
		...
		<div className={cn(styles.wrapper(), className)} style={style}>
			<div className={innerClass}>
				...
				<div className={styles.titlebar()}>
					...
					{titlebarButtons && (
						<div className={styles.titlebarButtons()}>{titlebarButtons}</div>
					)}
				</div>
				...
				{buttons && buttons.length > 0 && (
					<div className={styles.buttonGroup()}>
						{buttons.map(
							(
								btn: { label: string; onClick?: () => void; primary?: boolean },
								idx: number,
							) => (
								<Win95Button
									key={idx}
