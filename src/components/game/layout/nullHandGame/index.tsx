...
export function NullHandGame({
	...
	const [localRankings, setLocalRankings] =
		useState<UserRanking[]>(initialRankings);
	...
	useEffect(() => {
		if (
			...
			const updated = localRankings.map((ranking) => {
				const scoreAdd =
					currentScores.find((s) => s.userId === ranking.userId)?.points || 0;
				return {
					...ranking,
						...
						>
							...
							{phase === "BATTLE" && (
								<BattlePhase
									...
									onSubmit={() =>
										selectedHand && handleSetGuestHand(selectedHand)
									}
									hostName={hostName}
									hostFaceIcon={hostUser?.user?.faceIcon}
									currentScores={currentScores}
