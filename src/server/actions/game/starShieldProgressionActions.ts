...
export async function updateLoadout(params: {
	...
	const specialIds = new Set(
		existing.specialAttacks.map((a) => a.specialAttackId),
	);
	...
	if (params.selectedNormalAttackId !== undefined) {
		if (
			params.selectedNormalAttackId !== null &&
			!normalIds.has(params.selectedNormalAttackId)
		) {
	...
	if (params.selectedSpecialAttackId !== undefined) {
		if (params.selectedSpecialAttackId !== null) {
			if (params.selectedSpecialAttackId === "all_destruction") {
				if (maxHeal < 6)
					return { ok: false, error: "全部破壊はヒール Lv.  max で獲得" };
...
async function ensureProgress(
	userId: string,
): Promise<{ totalTypingCount: number }> {
...
/** 通常攻撃スキルを解放 */
export async function purchaseNormalAttackUnlock(
	techniqueId: string,
): Promise<{ ok: boolean; error?: string }> {
	const user = await getAuthenticatedUser();
	...
	if (techniqueId === "red")
		return { ok: false, error: "red は初期所持のため購入不可" };
...
export async function purchaseSpecialAttackLevelUp(
	...
	const user = await getAuthenticatedUser();
	if (specialAttackId !== "spread")
		return { ok: false, error: "all_destruction はレベルアップ対象外" };
...
export async function purchaseStarHpLevelUp(
	targetLevel: 2 | 3 | 4 | 5,
): Promise<{ ok: boolean; error?: string }> {
	const user = await getAuthenticatedUser();
	...
	const { totalTypingCount, starHpLevel } = await ensureProgressWithStarHp(
		user.id,
	);
	if (starHpLevel >= targetLevel)
		return { ok: false, error: "既にそのレベル以上です" };
	if (starHpLevel !== targetLevel - 1)
		return { ok: false, error: "前のレベルを先に購入してください" };
	if (totalTypingCount < cost)
		return { ok: false, error: "typing 数が不足しています" };
...
/** ヒールを解放 */
export async function purchaseHealUnlock(): Promise<{
	ok: boolean;
	error?: string;
}> {
	const user = await getAuthenticatedUser();
...
/** ヒールのレベル上げ */
export async function purchaseHealLevelUp(
	targetLevel: 2 | 3 | 4 | 5 | 6,
): Promise<{ ok: boolean; error?: string }> {
	const user = await getAuthenticatedUser();
