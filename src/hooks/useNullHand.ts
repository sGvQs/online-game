...
export function useNullHand({
	...
	const [jankenEvent, setJankenEvent] = useState<JankenEventWithGuests | null>(
		null,
	);
	 ...
	 * CHOICE → BATTLE フェーズへ
	 */
	const handleSetHostChoice = useCallback(
		async (choice: HostChoice) => {
	 ...
	 * ゲストの手を設定
	 */
	const handleSetGuestHand = useCallback(
		async (hand: HandType) => {
			if (!jankenEvent || isProcessing) return;
			play("select");
			setIsProcessing(true);
	...
	useEffect(() => {
		if (!matchIdRef.current) return;
		if (phase !== "CHOICE" && phase !== "RESULT" && phase !== "GAME_OVER")
			return;
