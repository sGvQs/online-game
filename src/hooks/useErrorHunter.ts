...
export function useErrorHunter({
	...
	const [winnerFaceIconPath, setWinnerFaceIconPath] = useState<string | null>(
		null,
	);
	 ...
	 */
	const setupAppearanceTimer = useCallback(
		(appearanceAt: Date | string) => {
			// 既存タイマーをクリア
			if (timerRef.current) {
			...
			if (typeof appearanceAt === "string") {
				...
				const dateString = appearanceAt.endsWith("Z")
					? appearanceAt
					: `${appearanceAt}Z`;
	...
	const updateLocalStateForClosedError = useCallback(
		(eventId: string, closedBy: string | null, closedAt: Date) => {
			setMatch((prev) => {
				if (!prev) return null;

				const newEvents = prev.errorEvents.map((e) =>
					e.id === eventId ? { ...e, closedAt, closedBy } : e,
				);
	 ...
	 */
	const saveErrorClickToDb = useCallback(
		async (eventId: string, matchId: string) => {
			try {
				...
				if (!isSuccess) {
					...
					console.log(
						`[saveErrorClickToDb] DB check failed for event ${eventId}, but UI already updated via broadcast`,
					);
				}
	...
	const handleClickError = useCallback(
		async (eventId: string) => {
			...
			updateLocalStateForClosedError(
				eventId,
				optimisticEvent.userId,
				optimisticEvent.createdAt,
			);
	...
	useEffect(() => {
		...
		broadcastChannel.on(
			"broadcast",
			{ event: "click-error" },
			(payload: BroadcastPayload<ErrorClickPayload>) => {
