...
import { GAME_DURATION_SECONDS } from "@/constants/starShieldGame/gameConfig";
import type { SpecialAttackLevel } from "@/constants/starShieldGame/gameConfig";
import {
	DIALOGUES,
	pickRandomDialogue,
} from "@/constants/starShieldGame/dialogues";
...
export function useStarShield({
	...
	const playVoice = useCallback(
		(key: string) => playVoiceRef.current(key as Parameters<typeof play>[0]),
		[],
	);
	...
	/** 接触爆発アニメ用 */
	const [contactExplosion, setContactExplosion] = useState<{
		x: number;
		y: number;
		asteroidId: string;
	} | null>(null);
	...
	useEffect(() => {
		autoAimNearestRef.current = process.env.NODE_ENV === "development";
	}, []);
	useEffect(() => {
	...
	const stableEndGame = useCallback(
		(result: GameResult) => endGameRef.current(result),
		[],
	);
	...
	useEffect(() => {
		...
		const { effectiveStartedAt, initialRemaining } = resolveStartTime(
			startedAt,
		...
		const interval = setInterval(() => {
			...
			if (remaining <= 0) {
				...
				if (!isShooter || gameEndedRef.current || difficulty === "ABYSS")
					return;
				endGame("CLEARED");
			}
		}, 1000);
	...
	const onKeyDown = useCallback(
		(e: KeyboardEvent) => {
			...
			if (selectedNormalAttack != null)
				payload.technique = selectedNormalAttack;
			if (isLastChar) {
