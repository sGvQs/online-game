...
function destroyNonBossAsteroids(
	...
	const toDestroy = ctx.asteroidsRef.current.filter(
		(a) => !a.destroyedAt && !a.isBoss,
	);
	...
	const nextAsteroids = ctx.asteroidsRef.current.map((a) =>
		ids.has(a.id) ? { ...a, destroyedAt: now, hp: 0 } : a,
	);
	ctx.asteroidsRef.current = nextAsteroids;
	ctx.setAsteroids(nextAsteroids);
	const nextScore = {
		...ctx.scoreRef.current,
		destroyed: ctx.scoreRef.current.destroyed + count,
	};
...
export function useShooterActions({
	...
	const handleHealEffect = useCallback(
		(payload: FirePayload, now: number) => {
			...
			const recovery =
				LEVEL_HEAL_RECOVERY[payload.healLevel as 1 | 2 | 3 | 4 | 5 | 6];
	...
	const handleSpreadSpecialAttack = useCallback(
		(payload: FirePayload, now: number) => {
			const specialAttack: SpecialAttackChoice =
				...
				payload.specialAttack in SPECIAL_ATTACK_BULLET_COUNT
	...
	const handleNormalAttack = useCallback(
		(payload: FirePayload, now: number) => {
			const aim = aimRef.current;
			const dir = aimToDirection(
				aim.x,
				aim.y,
				DINO_X,
				DINO_Y + BULLET_ORIGIN_Y_OFFSET,
			);
			if (!dir) return;
			const centerAngle = aimToCenterAngle(
				aim.x,
				aim.y,
				DINO_X,
				DINO_Y + BULLET_ORIGIN_Y_OFFSET,
	...
	const onReceiveFire = useCallback(
		(payload: FirePayload) => {
			...
			if (autoAimNearestRef.current) {
				const nearestPos = findNearestAsteroidPosition(
